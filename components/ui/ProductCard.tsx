...
const ProductCard: React.FC<ProductCardProps> = ({ product, className }) => {
    ...
    return (
        <Link to={`/product/${product.id}`} state={{ from: location.pathname + location.search }} className="block w-full h-full">
            <motion.div
                className={cn("group relative w-full aspect-[3/4] cursor-pointer bg-gunmetal border border-gunmetal hover:border-olive transition-all duration-300", className)}
                whileHover="hover"
                initial="rest"
            >
                {/* Image Container */}
                <div className="relative w-full h-full overflow-hidden bg-black/40">
                    ...
                    >
                        <img
                            ...
                            alt={product.title}
                            className={cn(
                                "w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-300",
                    ...
                    {!product.inStock && (
                        <div className="absolute top-4 left-4 z-20">
                            <div className="bg-alert text-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest border border-red-500/50 backdrop-blur-sm transform -rotate-2">
                                OUT OF STOCK
                            </div>
                        </div>
                    )}
                ...
                <div className="absolute bottom-0 left-0 w-full p-5 z-20">
                    <div className="mb-2">
                        <span className="text-olive text-[10px] uppercase tracking-widest font-bold flex items-center gap-1">
                            <Shield size={10} />
                    ...
                    <div className="flex justify-between items-end mt-3 border-t border-white/10 pt-3 group-hover:border-olive/30 transition-colors">
                        ...
                        <span className="text-brass font-display font-bold text-xl tracking-wide">
