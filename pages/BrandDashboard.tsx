...
export const BrandDashboard: React.FC = () => {
   ...
   const [brandProfile, setBrandProfile] = useState<{ id: string, name: string } | null>(null);

   useEffect(() => {
      const fetchProfile = async () => {
         try {
            const { artist } = await artistApi.getMyProfile();
            setBrandProfile({ id: artist.id, name: artist.user.fullName });
         } catch (error) {
   ...
   // Derived Data
   const brandProducts = products.filter(p => brandProfile && p.manufacturerId === brandProfile.id);
   const totalEarnings = brandProducts.reduce((sum, p) => sum + (p.inStock ? 0 : p.price), 0);
   const totalViews = brandProducts.length * 1243; // Mock views
   ...
   const handleSubmit = (e: React.FormEvent) => {
      ...
      if (selectedProduct) {
         ...
         const newProduct: Product = {
            ...
            manufacturerId: brandProfile?.id || '',
            manufacturerName: brandProfile?.name || '',
   ...
   return (
      <div className="pt-32 pb-12 max-w-7xl mx-auto px-4">
         ...
         <div className="flex justify-between items-center mb-12">
            <div>
               <h1 className="font-serif text-4xl text-white">Brand Studio</h1>
               <p className="text-stone-500 mt-1">Welcome back, {brandProfile?.name || 'Brand'}</p>
            </div>
            <button
            ...
            >
               <Upload size={16} /> Upload New Item
            </button>
            <Link
               to="/brand/profile"
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div className="bg-stone-900 p-6 border border-stone-800">
               <div className="flex justify-between items-start mb-4">
                  <div>
                     <p className="text-stone-500 text-xs uppercase tracking-widest">Catalog Size</p>
                     <h3 className="text-3xl text-white font-serif mt-2">{brandProducts.length} Items</h3>
                  </div>
         ...
         <h2 className="font-serif text-2xl text-white mb-6">Catalog Management</h2>

         {/* Products Table */}
         <div className="bg-stone-900 border border-stone-800 overflow-hidden rounded-sm">
            {brandProducts.length === 0 ? (
               <div className="p-12 text-center text-stone-500">
                  <p className="mb-4">Your catalog is currently empty.</p>
                  <button onClick={handleOpenUpload} className="text-amber-500 hover:underline">Upload your first item</button>
               </div>
            ) : (
                        ...
                        {brandProducts.map(p => (
                           <tr key={p.id} className="hover:bg-stone-800/50 transition-colors">
                              <td className="p-4 flex items-center gap-4">
                                 <img src={p.imageUrl} alt={p.title} className="w-12 h-12 object-cover rounded-sm border border-stone-700" />
                                 <div>
                                    <span className="block text-white font-medium">{p.title}</span>
                                    <span className="text-xs text-stone-500">{p.year} • {p.caliber}</span>
                                 </div>
                              </td>
                              <td className="p-4">{p.category}</td>
                              <td className="p-4">
                                 <span className={`px-2 py-1 text-xs rounded border ${p.inStock ? 'border-green-900 text-green-500 bg-green-900/10' : 'border-stone-700 text-stone-500 bg-stone-800'}`}>
                                    {p.inStock ? 'Active' : 'Sold'}
                                 </span>
                              </td>
                              <td className="p-4 font-mono">{convertPrice(p.price)}</td>
                              <td className="p-4">
                                 <div className="flex gap-3">
                                    <button onClick={() => handleOpenDetails(p)} title="View Details" className="text-stone-400 hover:text-tangerine transition-colors"><Eye size={18} /></button>
                                    <button onClick={() => handleOpenEdit(p)} title="Edit" className="text-stone-400 hover:text-amber-500 transition-colors"><Edit2 size={18} /></button>
                                    <button onClick={() => handleDelete(p.id)} title="Delete" className="text-stone-400 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                                 </div>
                              </td>
                           </tr>
                        ))}
            ...
            <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
               <div className="bg-stone-900 border border-stone-700 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg shadow-2xl animate-fade-in">
                  <div className="sticky top-0 bg-stone-900 border-b border-stone-800 p-6 flex justify-between items-center z-10">
                     <h2 className="font-serif text-2xl text-white">{selectedProduct ? 'Edit Catalog Item' : 'Add New Item'}</h2>
                  ...
                  <form onSubmit={handleSubmit} className="p-6 space-y-6">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                           ...
                           <div>
                              <label className="block text-xs uppercase text-stone-500 mb-1">Caliber / Type</label>
                              <input required type="text" placeholder="e.g. 9mm, .223 Rem" value={formData.caliber} onChange={e => setFormData({ ...formData, caliber: e.target.value })} className="w-full bg-stone-950 border border-stone-700 p-3 text-white focus:border-amber-500 outline-none" />
                           </div>
                           <div>
                              <label className="block text-xs uppercase text-stone-500 mb-1">Action / Capacity</label>
                              <input required type="text" placeholder="e.g. Semi-Auto, 15 Rounds" value={formData.capacity} onChange={e => setFormData({ ...formData, capacity: e.target.value })} className="w-full bg-stone-950 border border-stone-700 p-3 text-white focus:border-amber-500 outline-none" />
                           </div>
                        </div>

                        <div className="space-y-4">
                           <div>
                              <label className="block text-xs uppercase text-stone-500 mb-1">Product Image</label>
            ...
            <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
               <div className="bg-stone-900 border border-stone-700 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-lg shadow-2xl flex flex-col md:flex-row animate-fade-in">
                  ...
                  <div className="md:w-1/2 bg-black relative">
                     <img src={selectedProduct.imageUrl} alt={selectedProduct.title} className="w-full h-full object-contain" />
                  ...
                  <div className="md:w-1/2 p-8 flex flex-col">
                     <div className="flex justify-between items-start mb-6">
                        <div>
                           <h2 className="font-serif text-3xl text-white mb-1">{selectedProduct.title}</h2>
                           <p className="text-amber-500 text-sm uppercase tracking-widest">{selectedProduct.manufacturerName}</p>
                        </div>
                     ...
                     <div className="space-y-6 flex-1">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                           <div>
                              ...
                              <p className="text-white font-mono text-lg">{convertPrice(selectedProduct.price)}</p>
                           </div>
                           <div>
                              <p className="text-stone-500 text-xs uppercase">Status</p>
                              <p className={selectedProduct.inStock ? "text-green-500" : "text-stone-400"}>
                                 {selectedProduct.inStock ? "Available" : "Sold Out"}
                              </p>
                           </div>
                           <div>
                              <p className="text-stone-500 text-xs uppercase">Caliber</p>
                              <p className="text-stone-300">{selectedProduct.caliber}</p>
                           </div>
                           <div>
                              <p className="text-stone-500 text-xs uppercase">Capacity</p>
                              <p className="text-stone-300">{selectedProduct.capacity}</p>
                           </div>
                        </div>

                        <div>
                           <p className="text-stone-500 text-xs uppercase mb-2">Description</p>
                           <p className="text-stone-300 text-sm leading-relaxed">{selectedProduct.description}</p>
                        </div>

                        <div className="bg-stone-950 p-4 border border-stone-800 rounded">
                           <h4 className="text-white font-serif mb-3">Item Performance</h4>
                     ...
                     <div className="pt-6 mt-6 border-t border-stone-800 flex justify-end gap-3">
                        <button onClick={() => handleDelete(selectedProduct.id)} className="flex items-center gap-2 px-4 py-2 border border-stone-700 text-stone-400 hover:text-red-500 hover:border-red-900 transition-colors text-xs uppercase">
                           <Trash2 size={14} /> Delete
                        </button>
                        <button onClick={() => { setIsDetailsOpen(false); handleOpenEdit(selectedProduct); }} className="flex items-center gap-2 px-6 py-2 bg-stone-800 hover:bg-stone-700 text-white transition-colors text-xs uppercase">
